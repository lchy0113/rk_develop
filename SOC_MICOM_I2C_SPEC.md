...
| b2     | FACTORY_ACTIVE  | 공장 모드 활성 상태(1=ON, 0=OFF          |
...
|------|-------|-----------------------------------------|
| 0x00 | OK    | 오류 없음(정상 상태)                    |
| 0x01 | INVAL | 잘못된 인자/범위 입력                   |
| 0x02 | BUSY  | Micom busy 로 인해 명령 거부            |
| 0x03 | I²C   | 내부 I²C 오류 발생                      |
...
| 0x52 | EVENT_POP    | RO  | 4B  | 읽을 때마다 이벤트 1건 POP → [CODE_L][CODE_H][STATE][SEQ] |
...
조명 모듈 연동 여부와 관계없이,  
월패드 제품의 공장(Facotry) 모드 상태를 제어하기 위한 명령어를 정의한다.  
SoC는 이 명령어를 통해 Micom의 공장 모드를 설정(SET), 해제(UNSET), 조회(GET)할 수 있다.  
...
### REG: FACTORY_MODE_CMD(0x30)

| 항목             | 값 / 유형                                                                                 | 설명                                             |
| ---------------- | ----------------------------------------------------------------------------------------- | ------------------------------------------------ |
| **주소(OPCODE)** | 0x30                                                                                      | Micom 공장 모드 설정/해제 명령                   |
| **DATA**         | 0x00 또는 0x01                                                                            | 0x00 = 공장 모드 해제 (UNSET)<br>0x01 = 공장 모드 설정 (SET) |
| **응답 방식**    | STATUS(0x31) 레지스터 읽기                                                                | 별도의 응답 프레임 없이, SoC가 0x31 레지스터를 직접 읽어 상태를 확인  |
| **동작 방식**    | SoC가 0x30 명령을 전송하면, Micom은 내부 상태를 변경하고 `FACTORY_TEST_MODE_STATUS(0x31)` 레지스터 값을 업데이트한다. |                      |
| **비고**         | 설정/해제 후 STATUS(0x20).b2 비트가 자동 갱신되며, 드라이버의 `hw_test_mode` 값이 갱신됨. |                                                  |
...
### REG: FACTORY_MODE_STATUS(0x31)

| 항목       | 값 / 유형                                                   | 설명                   |
| ---------- | ----------------------------------------------------------- | ---------------------- |
| **주소**   | 0x31                                                        | Micom 공장 모드 상태 조회 레지스터 |
| **DATA**   | 1B (0x00 또는 0x01)                                         | 현재 공장 모드 상태    |
| **설명**   | SoC가 0x31 레지스터를 읽으면, Micom은 현재 공장 모드 상태(0=OFF, 1=ON)를 반환한다. | |
...
| 구분                       | I²C 전송                | 설명                               |
| -------------------------- | ----------------------- | ---------------------------------- |
| **공장 모드 설정 (SET)**   | SoC→Micom : `A5 30 01`  | 공장 모드 활성화 (`STATUS.b2=1`)   |
| **공장 모드 해제 (UNSET)** | SoC→Micom : `A5 30 00`  | 공장 모드 비활성화 (`STATUS.b2=0`) |
| **공장 모드 조회 (GET)**   | SoC→Micom : `A5 31`     | 현재 공장 모드 상태(ON=1) 반환     |
