...
| **응답(OPCODE)** | 0xB0                                                                 | Micom 현재 공장 모드 상태 반환 (0=OFF, 1=ON)     |
| **동작 방식**    | SoC가 0x30 명령을 전송하면, Micom은 내부 상태를 변경한 뒤 현재 공장 모드 상태(0/1)를 응답으로 회신한다. |               |
| **비고**         | 설정/해제 동작 후 STATUS(0x20).b2 비트가 자동 갱신됨.                |                                                  |
...
| 항목             | 값 / 유형                                        | 설명                                   |
| ---------------- | ------------------------------------------------ | -------------------------------------- |
| **주소(OPCODE)** | 0x31                                             | Micom 공장 모드 상태 조회 명령         |
| **DATA**         | 없음                                             | —                                      |
| **응답(OPCODE)** | 0xB0                                             | 현재 공장 모드 상태 반환 (0=OFF, 1=ON) |
| **동작 방식**    | SoC가 0x31 명령을 전송하면, Micom은 현재 공장 모드 상태를 즉시 회신한다. |                |
...
| 구분                       | I²C 전송                                         | 설명                               |
| -------------------------- | ------------------------------------------------ | ---------------------------------- |
| **공장 모드 설정 (SET)**   | SoC→Micom : `A5 30 01`<br>Micom→SoC : `A5 B0 01` | 공장 모드 활성화 (`STATUS.b2=1`)   |
| **공장 모드 해제 (UNSET)** | SoC→Micom : `A5 30 00`<br>Micom→SoC : `A5 B0 00` | 공장 모드 비활성화 (`STATUS.b2=0`) |
| **공장 모드 조회 (GET)**   | SoC→Micom : `A5 31`<br>Micom→SoC : `A5 B0 01`    | 현재 공장 모드 상태(ON=1) 반환     |
