...
## 예제
 ...
 - cd를 예를 들어 설명하면, CD의 샘플링 레이트가 44.1kHz이므로, 초당 *44100*번의 샘플링이 있으며,   
   샘플링된 신호는 *16비트*로 양자화된다.   
   1초에 전송해야할 데이터는 44100 x 16 = 705,600 비트 이다. (모노)  
   만약 스테레오의 경우, 왼쪽과 오른쪽 마이크를 통해서 샘플링 된 신호가 각각 존재하므로,   
   1초에 전송해야 할 데이터는 모노의 *2배*가 된다.    
   44100 x 16 x 2 = 1,411,200 비트가 된다. 
   즉, CD음원을 전송하는 경우, I2S의 데이터 전송 속도는 1.411 Mbits/s가 된다.   
 ...
 * PCM 데이터 처리량 (비트레이트;bit rate)  
 초당 데이터 량 = 샘플링 레이트 * 채널수 * 비트(비트 깊이)  
    705,600(bit) = 22,050        * 2      * 16  
  
     즉, 약 705 Kbps가 된다. 이 값이 1초 당, PCM 에서 처리되는 데이터 양이다. 그러나 MP3 로 압축되면서 705 Kbps 값이 56 Kbps 값으로 약 12 배 차이가 난다.  
